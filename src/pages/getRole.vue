    ...
    <div class="body">
      ...
      <section>
        <form class="wrapper" @submit.prevent="send()">
          <input v-model="value" type="text" placeholder="Значение" required>
...
export default {
  ...
  methods: {
    async send() {
      await axios.get(`http://localhost:5000/roles/${this.value}`, {
      })
      .then(response => {
      ...
      if(this.error.message == 'Network Error') {
        ...
        let div = document.getElementById('validate');
        div.classList.add('denied')
        this.send_error = 'Такой роли не существует!';
      }else {
        let div = document.getElementById('validate');
        div.classList.toggle('access');
        this.send_error = 'Роль успешно получена!';
      }
    }
  },
};
